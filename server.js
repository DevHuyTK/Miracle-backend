...
io.on("connection", async (socket) => {
  ...
  socket.on("join", async ({ token, userIds }) => {
    ...
    for (let userId of userIds) {
      const unique = [verified._id.toString(), userId.toString()].sort((a, b) =>
        a < b ? -1 : 1
      );
  ...
  socket.on("follow-user", async ({ token, userId }) => {
    ...
    // let resultFollower = await getFollowerPopulate( verifiedUser._id);
    // console.log(resultFollower);
    // let resultFollowing = await getFollowingPopulate(verifiedTargetUser._id);
    // console.log(resultFollowing);
      ...
      if (
        ...
        if (
          verifiedUser.user_liked_you.findIndex(
            (item) => item._id.toString() === verifiedTargetUser._id.toString()
          ) !== -1
        ) {
        ...
        const matchingList_1 = await Promise.all(
          [...user.matching_list].map(async (item) => {
            const unique = [verified._id.toString(), item._id.toString()].sort(
              (a, b) => (a < b ? -1 : 1)
            );
            const roomId = `${unique[0]}-${unique[1]}`;
        ...
        const matchingList_2 = await Promise.all(
          [...targetUser.matching_list].map(async (item) => {
            const unique = [userId.toString(), item._id.toString()].sort(
              (a, b) => (a < b ? -1 : 1)
            );
            const roomId = `${unique[0]}-${unique[1]}`;
            const message = await Chat.findOne(
        ...
        if (
          ...
          await User.findByIdAndUpdate(userId, {
            user_liked_you: [...targetUser.user_liked_you, verifiedUser],
          });
        }
      }
  ...
  socket.on("seen-message", async ({ token, userId }) => {
    ...
    const returnedMatchingList = await Promise.all(
      [...verifiedUser.matching_list].map(async (item) => {
        const unique = [verifiedUser._id.toString(), item._id.toString()].sort(
          (a, b) => (a < b ? -1 : 1)
        );
        const roomId = `${unique[0]}-${unique[1]}`;
        const message = await Chat.findOne(
