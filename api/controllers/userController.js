...
module.exports.index = async (req, res) => {
	try {
		...
		const returnedFollowingList = await Promise.all(
			[...user.following_list].map(async (item) => {
		...
		res.json({
			...
			data: {
				...returnedUser,
				following_list: returnedFollowingList,
			},
		});
	} catch (error) {
		return res.json({
...
module.exports.getUsers = async (req, res) => {
	try {
		User.find({ active: true }, { __v: 0, password: 0 }).exec(
			(err, data) => {
				if (err) res.send(err);
...
module.exports.updateUser = async (req, res) => {
	try {
		const data = req.body;
		const { email, phone, full_name, age, address, gender } = data;
		if (
			gender !== null &&
			gender !== undefined &&
			parseInt(gender) !== 1 &&
